...
if alpha > 0
    W = alpha*W;
    DCol = full(sum(W,2));
    D = spdiags(DCol,0,nSmp,nSmp);%产生一个nSmp×nSmp稀疏矩阵D
    L = D - W;%拉普拉斯矩阵
...
if nRepeat == 1
    ...
    if isempty(maxIter)
        ...
        if isfield(options,'Converge') && options.Converge
            objhistory = CalculateObj(X, U, V, L);%构造目标函数
        end
    end
else
    if isfield(options,'Converge') && options.Converge
...
function [U, V] = NormalizeUV(U, V, NormV, Norm)
    ...
    if Norm == 2
        if NormV
            ...
            V = V*spdiags(norms.^-1,0,K,K);%产生一个稀疏矩阵，且内部元素为KK
